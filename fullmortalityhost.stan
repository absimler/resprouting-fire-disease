data{
  ...
  real Canker[N]; //individual level SOD symptom predictor
...
transformed parameters {
  ...
  for ( i in 1:N ) {
    p[i] = a + vary_a[Plot[i]] + bdbh * LiveBAc[i] + bsymp*Canker[i];
...
generated quantities{
  ...
  for ( i in 1:N ) {
    pq[i] = a + vary_a[Plot[i]] + bdbh*LiveBAc[i] + bsymp*Canker[i];
