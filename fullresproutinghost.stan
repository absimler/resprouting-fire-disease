data{
    ...
    real Prsim[Nsims]; //sim data for Pr infection
    real Sympsim[Nsims]; //sim data for canker symptom
}
parameters{
...
transformed parameters {
    ...
    for ( i in 1:N ) {
  mu[i] = exp(a + vary_a[Plot[i]] + bdbh*LiveBAc[i] + btop*TopkillBA[i] + bPr*Pr[i] + bsymp*symptom06[i] ); //individual predictors + plot-level predictor component inside varying intercept
    }
...
for ( q in 1:Nsims ) {
    mu_sims[q] = exp(a + bdbh*dbhsim[q] + btop*topkillsim[q] + bSOD*DeadSODsim[q] + bfire*DeadFiresim[q] + bPr*Prsim[q] + bsymp*Sympsim[q]);
